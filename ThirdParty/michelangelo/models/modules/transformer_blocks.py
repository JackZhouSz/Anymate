...
from ThirdParty.michelangelo.models.modules.checkpoint import checkpoint
    ...
    def forward(self, qkv, mask=None):
        ...
        if self.flash:
            ...
            if mask is not None:
                ...
                # value_to_fill = torch.tensor(-1e9, dtype=torch.float16)
                weight.masked_fill_(mask == 0, -1e8)
            wdtype = weight.dtype
...
class ResidualCrossAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, data: torch.Tensor, mask=None):
        x = x + self.attn(self.ln_1(x), self.ln_2(data), mask)
